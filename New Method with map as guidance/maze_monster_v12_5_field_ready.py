...
Run:
    python maze_monster.py --guide known_route.json  # soft pre-drawn guide + DFS
...
ENABLE_MOTION = True
PROGRAM_VERSION = 'V12.5_FIELD_READY_TOPO_GUIDE_V2'
IR_FRONT_LEFT_ID = 1
SHARP_LEFT_ID = 2
SHARP_RIGHT_ID = 3
IR_FRONT_RIGHT_ID = 4
...
class TopologicalMazeGuide:
    ...
    def load(cls, path):
        ...
        guide_data = payload.get('topology_guide') if isinstance(payload, dict) else None
        if guide_data is None and isinstance(payload, dict) and str(payload.get('format', '')).startswith('robomaster_topology_guide'):
            guide_data = payload
        if not isinstance(guide_data, dict):
            raise ValueError('route file has no topology_guide; export it again with maze_designer V2')
        return cls(guide_data, source_path=path)

    def _validate(self):
        if str(self.data.get('format', '')) != 'robomaster_topology_guide_v2':
            raise ValueError(f"unsupported topology guide format: {self.data.get('format')!r}")
    ...
    def describe(self):
        ...
        return (
            ...
            f'score={best.score:+.1f} obs={best.observations} | target={marker}'
        )
...
def print_startup_info():
    print()
    print('==========================================================')
    print(' MAZE SOLVER V11.3 - DIGITAL IR ACTIVE-LOW')
    print('==========================================================')
    print()
...
def maze_main(guide_path=None, disable_guide=False):
    ...
    try:
        ...
        if bool(getattr(config, 'ENABLE_PREDRAWN_TOPOLOGY_GUIDE', True)) and (not disable_guide):
            if configured_guide_path and os.path.isfile(configured_guide_path):
                try:
                    ...
                    print(f'>>> MAP GUIDE loaded: {configured_guide_path}')
        ...
        while True:
            ...
            edge_event = edge_memory.observe(pose_x, pose_y, sharp_left_cm, sharp_right_cm, front_cm=nav_front_cm)
            ...
            if (not decision_event) and bool(open_state.get('exit_ready', False)):
...
def monster_cli():
    ...
    parser.add_argument('--guide', metavar='ROUTE.json', help='V2 route exported by maze_designer (default: known_route.json if present)')
