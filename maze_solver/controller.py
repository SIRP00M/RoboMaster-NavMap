...
class MotionController:
    ...
    def calculate_forward_speed(front_distance):
        ...
        return config.MIN_FORWARD_SPEED + ratio * (
            config.FORWARD_SPEED - config.MIN_FORWARD_SPEED
        )
    ...
    def set_heading_index(self, heading_index, pose_tracker=None):
        if self.heading_base_yaw is None:
            ...
            if current_yaw is None:
                return None
            self.initialize_heading(current_yaw, pose_tracker)
        if pose_tracker is not None:
            self._learn_heading_axis_from_pose(pose_tracker)
        ...
        self.heading_target_yaw = normalize_angle_deg(
            self.heading_base_yaw + self.heading_right_step_sign * 90.0 * step
        )
        return self.heading_target_yaw
    ...
    def calculate_heading_hold(self, current_yaw, pose_tracker, recover=False):
        ...
        max_z = (
            ...
            if recover else config.HEADING_MAX_Z_SPEED
        )
        desired_yaw_rate = clamp(error * config.HEADING_KP_Z, -max_z, max_z)
        return desired_yaw_rate / sign_map, error
    ...
    def update_wall_states(self, sharp_left_cm, sharp_right_cm):
        # >=20 cm is definitely an opening for exploration and must not also
        # participate as a centering wall.
        left_is_opening = (
            sharp_left_cm is not None and sharp_left_cm >= config.SIDE_OPEN_MIN_CM
        )
        right_is_opening = (
            sharp_right_cm is not None and sharp_right_cm >= config.SIDE_OPEN_MIN_CM
        )
        if left_is_opening:
            self.left_wall_present = False
        else:
            self.left_wall_present = self._update_wall_state(
                sharp_left_cm, self.left_wall_present
            )
        if right_is_opening:
            self.right_wall_present = False
        else:
            self.right_wall_present = self._update_wall_state(
                sharp_right_cm, self.right_wall_present
            )
    ...
    def calculate_center_owner(self, sharp_left_cm, sharp_right_cm):
        ...
        if owner_age >= config.CENTER_HOLD_SEC:
            ...
            if self.side_owner == "LEFT" and delta >= config.CENTER_TRIGGER_CM:
                ...
                self.side_owner_since = now
        correction = clamp(
            abs_delta * config.CENTER_KP_STRAFE, 0.0, config.CENTER_MAX_Y
        )
    ...
    def calculate_motion_control(
        ...
        # Filtered distance only; raw ADC spikes do not trigger escape.
        ...
        if left_wall:
            ...
            y_cmd = 0.0 if abs(error) <= config.SIDE_DEADBAND_CM else clamp(
